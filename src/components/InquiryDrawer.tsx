        ...
        <div className="p-4 sm:p-5 flex-1 overflow-y-auto space-y-3">
          {savedProducts.length === 0 ? (
            <div className="text-center py-16 px-4 space-y-4">
              ...
              <h3 className="font-bold text-slate-800 text-sm">لیست استعلام شما خالی است</h3>
              <p className="text-xs text-slate-500 max-w-xs mx-auto leading-relaxed">
                روی آیکون نشان‌کردن در کارت محصولات کلیک کنید تا مدل‌های دلخواه شما در این بخش برای مقایسه یا استعلام قیمت یکجا جمع‌آوری شوند.
              </p>
            </div>
          ) : (
            savedProducts.map((prod) => {
              ...
              return (
                <div
                  key={prod.id}
                  className="p-3 bg-slate-50 hover:bg-slate-100/70 border border-slate-200/80 rounded-2xl flex items-start gap-3 transition-colors"
                >
                  <img
                    src={prod.images[0]}
                    alt={prod.name}
                    className="w-16 h-16 rounded-xl object-cover bg-slate-200 shrink-0 cursor-pointer"
                    onClick={() => {
                      onOpenProduct(prod);
                      onClose();
                    }}
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-start justify-between gap-1">
                      <h4
                        className="font-bold text-xs text-slate-900 truncate hover:text-amber-800 cursor-pointer"
                        onClick={() => {
                          onOpenProduct(prod);
                          onClose();
                        }}
                    ...
                    <div className="flex items-center gap-2 mt-1 text-[11px] text-slate-500">
                      ...
                      <span>سایز: {prod.specs.unitSize || '-'}</span>
                    </div>
                    ...
                    <div className="mt-2.5 flex items-center justify-between">
                      <div className="flex items-center gap-1.5 bg-white border border-slate-200 rounded-lg p-0.5">
                        <button
                          ...
                          className="w-6 h-6 rounded bg-slate-50 hover:bg-slate-100 flex items-center justify-center text-slate-700"
                        >
                      ...
                      <span className="text-[11px] font-medium text-amber-800">
                        {prod.priceEstimate || 'استعلام قیمت روز'}
                      </span>
                    </div>
                  </div>
                </div>
          ...
          <div className="p-4 sm:p-5 border-t border-slate-200 bg-slate-50 space-y-3">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
              ...
              <a
                ...
                className="w-full flex items-center justify-center gap-1.5 py-2.5 px-3 rounded-xl bg-sky-600 hover:bg-sky-500 text-white text-xs font-bold shadow-md shadow-sky-900/20 transition-all"
              >
                <Send className="w-4 h-4" />
                <span>ارسال در تلگرام تاتار</span>
              </a>
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
              <a
                ...
                target="_blank"
                rel="noopener noreferrer"
                className="w-full py-2 px-3 rounded-xl bg-gradient-to-r from-pink-600 to-purple-600 hover:from-pink-500 hover:to-purple-500 text-white text-xs font-bold flex items-center justify-center gap-1.5 transition-all"
              >
                <Instagram className="w-3.5 h-3.5" />
                <span>دایرکت اینستاگرام ({SHOWROOM_INFO.instagramId})</span>
              </a>
